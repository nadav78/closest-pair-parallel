/*
 * An implementation of the divide and conquer algorithm for the closest pair of points problem.
 */
 ...
 * between any two pair of points in array p. Assumes that the array P[] is  
 * sorted according to x the coordinate.
 */
double closest_serial(struct Point *p, int n) {
    ...
    double new_min = min_distance(d, strip_closest(strip, j, d));
    free(strip);
